...
//			return;
		...
		$aMainTable['table2Field']=array(); //只针对date,day
		...
		foreach($aTables as $table => $aTable){
			...
			if(!empty($aTable['dimen'])&&is_array($aTable['dimen'])){
				foreach($aTable['dimen'] as $dimen){
					$key=$dimen['key'];
					if(isset($aTmpTables[$table]['dimen']['key'])&&in_array($key,$aTmpTables[$table]['dimen']['key']))continue;//dimen去重
					if($dimen['group']){
						...
						if($type != 'flash'){//为flash时不需要showField
							if(!in_array($key,$aMainTable['showField'])){
								$aMainTable['showField'][]=$key; //显示的字段 维度
				...
				foreach($aTable['metric'] as $metric){
					if($metric['show']){
						...
						if(!in_array($metric['key'],$aMainTable['showField'])){
							$aMainTable['showField'][]=$metric['key']; //显示的字段 指标
						}
						if(3==$metric['type'][0]){
							//去重
							if(!in_array($metric['key'],$aMainTable['showField'])){
						...
						switch($metric['type'][0]){
							...
							case 3:
								if(!in_array($metric['key'],$aMainTable['field3'])){
									$aMainTable['field3'][]=$metric['key']; //显示的字段 指标
								}

								if($table==$aMainTable['table']){//主表处理
									$key=$metric['type'][1][0];
...
//		print_r($type);
	...
	public function initInputAdvance($type){
		...
		$aMainTable['table'] = $table;//只设置一个主表
